		...
		SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float screenDepth350 = 1;
				float distanceDepth350 = abs((screenDepth350 - 1) / (100.0));
				...
				float eyeDepth64 = (1,_ZBufferParams);
				...
				float screenDepth434 = (1,_ZBufferParams);
				float distanceDepth434 = abs((screenDepth434 - 1) / (_FoamAmount));
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN) : SV_TARGET
			{
				...
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = (UNITY_NEAR_CLIP_VALUE >= 0) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float screenDepth350 = (1,_ZBufferParams);
				float distanceDepth350 = abs((screenDepth350 - 1) / (100.0));
				float4 ase_grabScreenPos = ASE_ComputeGrabScreenPos(screenPos);
				float4 ase_grabScreenPosNorm = ase_grabScreenPos / ase_grabScreenPos.w;
				float4 appendResult405 = (float4(_NormalMapsWavesSize , _NormalMapsWavesSize , 0.0 , 0.0));
